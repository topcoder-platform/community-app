...
docker build -t $TAG \
  ...
  # Credentials for TopGear space in Contentful CMS.
  --build-arg CONTENTFUL_TOPGEAR_CDN_API_KEY=\
    $CONTENTFUL_TOPGEAR_CDN_API_KEY \
  --build-arg CONTENTFUL_TOPGEAR_PREVIEW_API_KEY=\
    $CONTENTFUL_TOPGEAR_PREVIEW_API_KEY \
  --build-arg CONTENTFUL_TOPGEAR_SPACE_ID=\
    $CONTENTFUL_TOPGEAR_SPACE_ID \
