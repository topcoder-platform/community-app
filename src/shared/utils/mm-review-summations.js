...
function updateSubmissionEntryFromSubmission(
  ...
  return {
    ...
    url: _.isNil(url) ? baseEntry.url : url,
    virusScan: _.isNil(virusScan) ? baseEntry.virusScan : virusScan,
  ...
  normalizedRawSubmissions.forEach((submission, index) => {
    ...
    const updatedEntry = updateSubmissionEntryFromSubmission(
      memberEntry.submissionsMap.get(submissionId),
      {
        ...
        url: _.get(submission, 'url'),
        virusScan: _.get(submission, 'virusScan'),
        isLatest,
  ...
  const members = Array.from(membersByHandle.values()).map((memberEntry) => {
    const submissions = Array.from(memberEntry.submissionsMap.values())
      .map(submission => ({
        ...
        url: submission.url,
        virusScan: _.isNil(submission.virusScan) ? null : submission.virusScan,
        reviewSummations: [...submission.reviewSummations],
        reviewSummation: [...submission.reviewSummations],
      }))
      .sort(
        (a, b) => toTimestampValue(b.submissionTime)
