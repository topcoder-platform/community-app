    ...
    <div styleName="prizes-container">
      {
        _.range(prizeLength).map((index) => {
          ...
          return (
            <div key={rank} onClick={(event) => {event.preventDefault()} role="contentinfo" tabIndex={0} aria-label={`${rank}${getOrdinal(rank)} prize ${!_.isUndefined(prizes[index]) ? '$' : ''}${pair.join(' + ')}`} styleName="prize-fill">
