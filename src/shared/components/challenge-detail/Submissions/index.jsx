...
function renderSubmission(s) {
  return (
    <div styleName="submission" key={s.submissionId}>
      <a target="_blank" href={`${utils.config.URL.STUDIO}?module=DownloadSubmission&sbmid=${s.submissionId}`}>
        <img alt="" src={`${utils.config.URL.STUDIO}/studio.jpg?module=DownloadSubmission&sbmid=${s.submissionId}&sbt=small&sfi=1`} />
      </a>
      <div styleName="bottom-info">
        <div styleName="links">
          <a target="_blank" href={`${utils.config.URL.STUDIO}?module=DownloadSubmission&sbmid=${s.submissionId}`}>{`#${s.submissionId}`}</a>
          <a styleName="handle" target="_blank" href={`${utils.config.URL.BASE}/members/${s.submitter}`}>{s.submitter}</a>
        </div>
      ...
      </div>
      {
        submissions.map(s => (
          <div key={s.handle + s.submissionDate} styleName="row">
            <div styleName="col-1">
              <a styleName="handle" target="_blank" href={`${utils.config.URL.BASE}/member-profile/${s.handle}/develop`}>{s.handle}</a>
            </div>
