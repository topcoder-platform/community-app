        ...
        <div styleName={outStyleName}>
          <div styleName="completed-challenge">
            <header>
              ...
              <p styleName="subtrack-color">{underscoreReplace(challenge.subTrack)}</p>
              <p styleName="date-completed">{formatDate(challenge.submissionEndDate)}</p>
              ...
              <div styleName="challenge-details DATA_SCIENCE">
                { challenge.track === 'DATA_SCIENCE' && challenge.subTrack &&
                  <div styleName="marathon-score">
                    <p styleName="score">{ challenge.pointTotal || 0 }</p>
                    <p>Total Points</p>
                  </div> }
                    ...
                    <div styleName="tile-view">
                      { challenge.highestPlacement &&
                        <p styleName={challenge.wonFirst ? 'first-place place' : 'place'}>
                          { challenge.highestPlacement }
                          <span>{ getPlacementPostfix(challenge.highestPlacement) }</span> Place
                        </p>
                      }
                      { challenge.userStatus === 'NOT_FINISHED' &&
                        <p styleName="place">Didn&apos;t Finish</p> }
                      { challenge.userStatus === 'PASSED_SCREENING' &&
                        <p styleName="place">Passed Screening</p> }
                      { !challenge.highestPlacement && challenge.userStatus === 'PASSED_REVIEW' &&
                        <p styleName="place">Passed Review</p> }
                      { challenge.userStatus === 'COMPLETED' &&
                        <p styleName="place">COMPLETED</p> }
                      { challenge.userStatus === 'PASSED_REVIEW' &&
                        <div styleName="challenge-score">
                          <p styleName="score">{ percentage(challenge.userDetails.submissionReviewScore / 100) }</p>
                          <p styleName="last-child">Review Score</p>
                        </div> }
                              ...
                              {challenge.numImages > 0 &&
                                <div styleName="thumbnail-gallery">
                                  ...
                                  <div styleName="num-images">
                                    {challenge.submissionViewable &&
                                      <span>{challenge.numImages} IMAGES</span>
                                    }
                                    {!challenge.submissionViewable &&
                                      <span>No image is visible</span>
                                    }
                                  </div>
                                </div>
                              }
                ...
                <div styleName="invite-only-banner">
                  <div styleName="title">INVITE-ONLY CHALLENGE</div>
                  <InviteOnly />
                  <span>CHALLENGE INFORMATION IS CONFIDENTIAL<br />
